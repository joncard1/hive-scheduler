...
object Dispatcher {
  ...
  /** A function that spawns a single worker as a child of the dispatcher.
    ...
    */
  type WorkerFactory = (ActorContext[Command], Int) => ActorRef[Worker.Command]
  ...
  def apply(
    ...
    apply(
      pointsMemory,
      prospectsMemory,
      (ctx, i) => ctx.spawn(Worker(kernel, ctx.self), s"worker-$i")
    )

  /** Full factory for the dispatcher actor with an injectable worker factory.
    * Prefer the two-argument overload for production use; supply a custom
    * `workerFactory` in tests to decouple the Dispatcher from the Worker
    * implementation.
    *
    * @param pointsMemory
    *   A reference to the collection of points generated by the system. It is
