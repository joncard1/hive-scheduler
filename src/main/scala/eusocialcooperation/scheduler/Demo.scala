...
@main def Demo(): Unit = {
    ...
    //logger.debug("Final points: " + pointsData.map(p => s"(${p.value._1}, ${p.value._2}, ${p.value._3}), ${p.actorName}, ${p.sequenceNumber}, ${p.phase}").mkString(", "))
...
// 3. Change to JavaFX. I just prefer it
// 4. Add a 2D plot of the prospects overlaid on the explorer points. I always change the plot to show that, anyway; might as well.
// 5. Move the image generation to a button so that I can overwrite the images if I want to.
// 6. Figure out how to do legends.
// 7. Figure out how to do a colorbar legend with the color determined by the sequence #.
// 8. Make the distribution of preferences conigurable.
